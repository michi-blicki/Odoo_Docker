...
DEFAULT_REMOTE_HOST = "lxerp01"
DEFAULT_REMOTE_BACKUP_INDEX = "/srv/backup/odoo-prod/meta/backups_index.json"
DEFAULT_TEMP_RESTORE_DIR = "/srv/restore/odoo-test/tmp"
DEFAULT_RESTORE_LOG_FILE = "/srv/restore/odoo-test/odoo-test-restore.log"
DEFAULT_REMOTE_SSH_KEY = str(Path.home() / ".ssh/id_rsa")
DEFAULT_DOCKER_COMPOSE_FILE = "/home/blicki/docker/odoo-test/docker-compose.yml"
...
def list_backups(args):
    """List backups from production host."""
    logging.info("Loading backup index from remote host...")

    # 1. Lokales Temp-Verzeichnis sicherstellen
    os.makedirs(args.temp_restore_dir, exist_ok=True)
    local_index_path = os.path.join(args.temp_restore_dir, "backups_index.json")

    # 2. Remote-Datei holen per SCP
    scp_cmd = [
        "scp", "-i", args.remote_ssh_key,
        f"{args.remote_host}:{args.remote_backup_index}",
        local_index_path
    ]
    logging.debug(f"Kopiere Backup-Index mit: {' '.join(scp_cmd)}")
    run_cmd(scp_cmd)

    # 3. JSON laden
    logging.info(f"Lade lokale Kopie: {local_index_path}")
    index_data = load_json(local_index_path)
    ...
    print("\nVerfügbare Backups:\n")
    header = f"{'Nr.':<3} {'Timestamp':<28} {'Type':<10} {'Size(MB)':>9}  {'Image Tag':<30} {'SHA256':<12}"
    print(header)
    print("-" * len(header))
    ...
    for i, b in enumerate(backups, start=1):
        ...
        tag = b.get("image_tag", "")[:29]
        sha = b.get("sha256", "")[:10]
        print(f"{i:<3} {ts:<28} {bt:<10} {size:>9}  {tag:<30} {sha:<12}")
...
def prepare_restore_environment(args, restore_dir: str) -> dict:
    ...
    - docker-compose.yml auswerten
    - Test-Container stoppen
    - Backup-Archiv entpacken
    - meta.json laden
    - Pfade von odoo-prod -> odoo-test umschreiben
    - Verzeichnisse leeren (odoo_data, custom_addons, ...)
    """
    logging.info("Starte Schritt 4: Vorbereitung der Restore-Umgebung ...")

    # 4.1 docker-compose.yml laden
    compose_path = Path(args.docker_compose_file or "docker-compose.yml")
    if not compose_path.exists():
        logging.error(f"{compose_path} nicht gefunden – Abbruch.")
        sys.exit(1)
    ...
    # 4.2 Container stoppen inkl. Volumes und Orphans
    print("🛑 Stoppe evtl. laufende odoo-test Container ...")
    down_cmd = ["docker", "compose", "-f", str(compose_path),
    ...
    # 4.9: odoo_logs Volume leeren (ebenfalls mit sudo)
    if "odoo_logs" in volumes:
        logs_path = compose_data["volumes"]["odoo_logs"].get("driver_opts", {}).get("device")
        if logs_path and os.path.exists(logs_path):
            logging.info(f"Leere {logs_path} Volume (mit sudo) …")
            print(f"🧹 Leere {logs_path} (Host, sudo) …")

            try:
                run_cmd(["sudo", "rm", "-rf", os.path.join(logs_path, "*")])
                logging.info(f"Inhalt von {logs_path} erfolgreich gelöscht (sudo).")
                print(f"✅ Alle Inhalte von {logs_path} gelöscht (mit sudo).")
            except SystemExit:
                logging.error(f"Fehler beim Löschen von {logs_path} (mit sudo).")
                print(f"❌ Fehler beim Löschen von {logs_path} (mit sudo).")
                return
        else:
            logging.warning(f"Volume‑Pfad {logs_path} nicht gefunden oder None.")
            print(f"⚠️ Volume‑Pfad {logs_path} nicht gefunden oder None.")
...
def recreate_backup(args):
    ...
    # Index laden und Backup-Eintrag finden
    local_index_path = os.path.join(args.temp_restore_dir, "backups_index.json")
    index_data = load_json(local_index_path)
    ...
    # Backup-Archiv vom Remote holen
    ...
    scp_cmd = [
        "scp", "-i", args.remote_ssh_key,
        f"{args.remote_host}:{archive_remote}",
        local_archive_path
    ]
    run_cmd(scp_cmd)
    ...
    # === Phase C: Docker-Image prüfen / ggf. holen (Schritt 5) ===
    if image_remote and image_tag:
        local_image_path = os.path.join(restore_dir, Path(image_remote).name)
        desired_id = selected.get("image_id")
        local_id = get_local_image_id(image_tag)

        if local_id == desired_id:
            logging.info(f"Local docker image {image_tag} identical to required image from restore. No download required.")
            print(f"✅ Image {image_tag} ({desired_id[:20]}...) already exists. No download required.")
        else:
            logging.info(
                f"Image {image_tag} missing or does not match requirement of backup (local: {local_id}, required: {desired_id})."
            )
            try:
                logging.info(f"Delete local image with '{image_tag}' if available ...")
                subprocess.run(["docker", "rmi", "-f", image_tag], check=False,
                               stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                logging.info("Local image deleted.")
            except Exception as e:
                logging.warning(f"Error removing old image with tag '{image_tag}': {e}")

            print(f"⬇️  Download required Docker-Image from PROD ...")
            scp_cmd = [
                "scp", "-i", args.remote_ssh_key,
                f"{args.remote_host}:{image_remote}",
                local_image_path
            ]
            run_cmd(scp_cmd)

            logging.info("Load required image to local docker instance ...")
            docker_load_cmd = ["docker", "load", "-i", local_image_path]
            run_cmd(docker_load_cmd)
            logging.info(f"Docker image '{image_tag}' successfully loaded.")
            print(f"✅ Docker-Image '{image_tag}' successfully loaded.")
    else:
        logging.warning("No image_path or image_tag in Backup-entry found!")
...
def restore_database(args, meta, restore_dir):
    ...
    ----------------------------------------------------------
    6.1 Container starten
    6.2 Warten bis der Health-Status "healthy" ist (max. 20 Sek.)
    ...
    db_service = "odoo-test-db"  # aus docker-compose.yml
    compose_file = args.docker_compose_file
    db_user = meta["db"].get("user", "odoo")
    db_name = meta["db"].get("name", "odoo")

    # --- 6.1 PostgreSQL-Container starten ---
    print(f"🐘 Starte PostgreSQL-Container '{db_service}' ...")
    ...
    dump_gz = os.path.join(restore_dir, "odoo-db.sql.gz")
    ...
    import_psql = ["docker", "exec", "-i", db_service, "psql", "-U", db_user, "-q", "-d", db_name]
...
def menu_mode(args):
    ...
    # 2. Datei per SCP holen (gleiche Logik wie list_backups)
    scp_cmd = [
        "scp", "-i", args.remote_ssh_key,
        f"{args.remote_host}:{args.remote_backup_index}",
        local_index_path
...
def main():
    ...
    parser.add_argument("--remote-host", default=DEFAULT_REMOTE_HOST,
                        help=f"Hostname/IP of odoo-prod Host (Default: {DEFAULT_REMOTE_HOST})")
    parser.add_argument("--remote-backup-index", default=DEFAULT_REMOTE_BACKUP_INDEX,
                        help=f"Path of backups_index.json one Remote Host (Default: {DEFAULT_REMOTE_BACKUP_INDEX})")
    ...
    parser.add_argument("--remote-ssh-key", default=DEFAULT_REMOTE_SSH_KEY,
                        help=f"Path to private SSH-Key for SCP actions (Default: {DEFAULT_REMOTE_SSH_KEY})")
    parser.add_argument("--docker-compose-file", default=DEFAULT_DOCKER_COMPOSE_FILE,
                        help=f"Path to docker-compose.yml (Default: {DEFAULT_DOCKER_COMPOSE_FILE})")
